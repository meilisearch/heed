...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("1. Inserting 200 entries to create a multi-level B+Tree...");
    {
        ...
        for i in 0..200 {
            ...
            BTree::insert(&mut txn, &mut root, &mut db_info, key.as_bytes(), value.as_bytes())?;
        }
    ...
    println!("\n2. Deleting every other entry (100 total) to trigger rebalancing...");
    {
        ...
        for i in (0..200).step_by(2) {
            let key = format!("key_{:04}", i);
            BTree::delete(&mut txn, &mut root, &mut db_info, key.as_bytes())?;
        }
    ...
    println!("\n3. Verifying remaining entries...");
    {
        ...
        for i in 0..200 {
            let key = format!("key_{:04}", i);
            match BTree::search(&txn, root, key.as_bytes())? {
                Some(_) => {
                    if i % 2 == 0 {
