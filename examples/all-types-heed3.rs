...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let ret: Option<&[u8]> = db.get(&mut wtxn, "hello")?;
    ...
    let ret: Option<Hello> = db.get(&mut wtxn, "hello")?;
    ...
    let ret: Option<()> = db.get(&mut wtxn, "hello")?;

    println!("{:?}", ret);

    let ret: Option<()> = db.get(&mut wtxn, "non-existant")?;
    ...
    let rets: Result<Vec<(i64, _)>, _> = db.iter(&mut wtxn)?.collect();

    println!("{:?}", rets);

    // or iterate over ranges too!!!
    let range = 35..=42;
    let rets: Result<Vec<(i64, _)>, _> = db.range(&mut wtxn, &range)?.collect();
