...
impl PolyDatabase {
        ...
        DC: BytesDecode<'txn>,
    {
        ...
        match result {
            Ok(()) => {
                ...
                let data = DC::bytes_decode(data).map_err(|e| Error::Decoding(e))?;
                Ok(Some(data))
            }
        ...
        DC: BytesDecode<'txn>,
    {
        ...
        match cursor.move_on_prev() {
            Ok(Some((key, data))) => match (KC::bytes_decode(key), DC::bytes_decode(data)) {
                ...
                (Err(e), _) => Err(Error::Decoding(e)),
                (_, Err(e)) => Err(Error::Decoding(e)),
            },
            Ok(None) => Ok(None),
            Err(e) => Err(e),
        }
        ...
        R: RangeBounds<KC::EItem>,
    {
        ...
        let start_bound = match range.start_bound() {
            Bound::Included(bound) => {
                let bytes = KC::bytes_encode(bound).map_err(|e| Error::Encoding(e))?;
        ...
        KC: BytesEncode<'a>,
    {
        ...
        let prefix_bytes = KC::bytes_encode(prefix).map_err(|e| Error::Encoding(e))?;
